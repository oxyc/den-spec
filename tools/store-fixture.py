#!/usr/bin/env python3
"""Generate the store-v2 test vectors: a tiny store, and the values a correct reader gets out of it.

  python3 tools/store-fixture.py --build-store ../den-dataset/scripts/v2/build_store.py \
      --out-dir vectors

Writes `vectors/store-v2.store` (a few hundred KB) and `vectors/store-v2.json`.

`vectors/store-v1.*` are no longer generated: they are the last bytes a store-v1 writer produced, kept
frozen so a reader can prove it still reads a store published before `franchise` became a list.

## Why this exists

`wire/store-v2.md` is implemented twice: a Python writer in den-dataset and a Rust reader in den-core, in
...
def main():
    ...
    store = os.path.join(args.out_dir, "store-v2.store")
    ...
    expected = {
        "_": "Test vectors for den-spec wire/store-v2.md. Generated by tools/store-fixture.py.",
